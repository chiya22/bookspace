...
export default async function AdminBooksPage({ searchParams }: Props) {
  ...
  const tagsByBookId = await getTagsByBookIds(books.map((b) => b.id));

  const withCovers = books.map((b) => ({
    ...b,
    coverUrl: getNdlThumbnailUrl(b.isbn) || null,
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <h1 className="text-xl font-semibold text-zinc-900">蔵書管理</h1>
        <Link
      ...
      {withCovers.length === 0 ? (
        ...
        <ul className="divide-y divide-zinc-200 rounded-lg border border-zinc-200 bg-white">
          {withCovers.map((book) => (
            <li key={book.id} className="flex items-center gap-4 px-4 py-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <span className="ml-2 text-sm text-zinc-600">{book.author}／{book.publisher}</span>
                {book.tags.length > 0 && (
