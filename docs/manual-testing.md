...
任意: **`PAGINATION_PAGE_SIZE`** を設定すると、一覧画面の1ページあたりの件数（未設定時は 20）を変更できます。
...
- **トップ（`/`）**: 会員証QR と 貸出中一覧 が表示されていること。一覧が多い場合はページングで切り替え可能なこと。
- ヘッダーの **「アカウント」** をクリック → **名前・表示名・メール・会員証QR** が表示され、表示名を編集できること。
- ヘッダーの **「蔵書検索」** をクリック → 蔵書の検索・一覧・詳細（表紙・タグ・お気に入り・コメント等）が利用できること。一覧はページング対応。
...
6. **「受付・貸出」** → 貸出・返却処理画面、および **貸出履歴一覧**（`/reception/loans`）が利用できること。貸出履歴一覧ではキーワード検索・ページング・**返却依頼メール**（1件ごと）が利用できること。
7. **「管理」** → 管理メニュー（蔵書管理・利用者管理・棚卸のリンク）が表示されること。
8. **棚卸（`/admin/inventory`）**: 「在庫チェック履歴をクリア」でクリア後、ISBNでチェックし、未チェック一覧・最終クリア日時が表示されること。一覧はページング対応。
9. 蔵書管理・利用者管理・タグ管理の各一覧で **ページング** が動作すること（件数が多い場合）。必要に応じて環境変数 **`PAGINATION_PAGE_SIZE`**（未設定時は 20 件）で1ページあたりの件数を変更できること。
