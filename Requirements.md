...
- **棚卸**: 在庫チェック履歴のクリア、ISBN入力による在庫チェックの記録、在庫チェック履歴が存在しない書籍（未チェック）の一覧表示。最後に在庫チェック履歴をクリアした日時を画面上に表示・保持する。
...
| 貸出時 | 貸出完了の通知 | 当該利用者 |
| 返却時 | 返却完了の通知 | 当該利用者 |
| 管理者操作 | 返却依頼 | 管理者が指定した利用者（貸出1件ごと） |

- 通知手段は **メールのみ**（アプリ内通知は対象外）。
- **返却依頼メール**: 司書・管理者が **貸出履歴一覧画面**（`/reception/loans`）で、貸出中である履歴の行から送信する。1回の送信で **その貸出1件** に対応する利用者に送信する。メール本文には **借りている書籍情報（タイトル・著者）**・**貸出日**・**貸出日数** を含める。
...
| **一覧のページング** | 情報が一覧で表示される画面ではページングを行う。1ページあたりの最大表示件数は環境変数 **PAGINATION_PAGE_SIZE** で変更可能（未設定時は20件）。 |
...
- **通知**: 貸出・返却時の利用者向けメール、および貸出履歴一覧画面からの返却依頼メール（本文に書籍・貸出日・貸出日数を含む）を実装する。送信履歴はデータベースに保存する。
- **棚卸**: 在庫チェック履歴のクリア、ISBNによる在庫チェック記録、未チェック書籍の一覧表示、最終クリア日時の表示・保持。
- **一覧画面**: ページングを導入し、最大表示件数は環境変数 PAGINATION_PAGE_SIZE で変更可能とする。
